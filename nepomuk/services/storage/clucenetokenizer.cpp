//krazy:skip
/*
 * Modified version of StandardTokenizer.cpp for Nepomuk mostly to optimize for filename indexing
 * Copyright (C) 2008 Sebastian Trueg <trueg@kde.org>
 *
 * Based on StandardTokenizer.cpp from the CLucene package.
...
/*
 * Modified version for Nepomuk mostly to optimize for filename indexing
 * Changes:
 * - underscore is treated as space, ie. tokens are always divided at underscores
 * - try to always divide tokens at dots.
 *   FIXME: we should still allow the acronym detection
 *   FIXME: is it possible to extract both? for example email addresses. Here it would
 *          be nice to have both the full address as a token and the separated portions.
 */
...
** that's not an ALNUM can be considered not to break the
** span.  Callers should pass false if only ALNUM are acceptable. */
...
namespace Nepomuk {
    ...
    CLuceneTokenizer::CLuceneTokenizer(Reader* reader):
    ...
    CLuceneTokenizer::~CLuceneTokenizer() {
        _CLDELETE(rd);
    }

    int CLuceneTokenizer::readChar() {
    ...
    void CLuceneTokenizer::unReadChar() {
        rd->UnGet();
        rdPos--;
    }
    ...
    bool CLuceneTokenizer::next(Token* t) {
        int ch=0;
        while (!EOS) {
            ch = readChar();
    ...
    bool CLuceneTokenizer::ReadNumber(const TCHAR* previousNumber, const TCHAR prev,Token* t) {
    ...
    bool CLuceneTokenizer::ReadAlphaNum(const TCHAR prev, Token* t) {
        t->growBuffer(LUCENE_MAX_WORD_LEN+1);//make sure token can hold the next word
        StringBuffer str(t->_termText,t->bufferLength(),true); //use stringbuffer to read data onto the termText
        if (  str.len < LUCENE_MAX_WORD_LEN ){
            str.appendChar(prev);
    ...
    bool CLuceneTokenizer::ReadCompany(StringBuffer* _str, Token* t) {
        StringBuffer& str = *_str;
        const int32_t specialCharPos = rdPos;
        int ch=0;
