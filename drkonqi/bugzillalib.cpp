...
void BugzillaManager::setFeaturesForVersion(const QString& version)
{
    ...
    if (digits.count() > nVersionParts) {
        kWarning() << QString("Current Bugzilla version %1 has more than %2 parts. Check that this is not a problem.").arg(version).arg(nVersionParts);
    }
    int currentVersion = KDE_MAKE_VERSION(digits.at(0).toUInt(),
