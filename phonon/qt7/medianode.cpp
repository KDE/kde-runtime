...
MediaNode::MediaNode(NodeDescription description, QObject *parent)
    : QObject(parent), m_audioNode(0), m_description(description), m_owningMediaObject(0)
{
}

MediaNode::MediaNode(NodeDescription description, AudioNode *audioPart, QObject *parent)
    : QObject(parent), m_audioNode(audioPart), m_description(description)
{
}
...
AudioGraph *MediaNode::audioGraph()
{
    if (!m_audioNode)
        return 0;
    return m_audioNode->m_audioGraph;
}
...
void MediaNode::notify(const MediaNodeEvent *event, bool propagate)
{
    ...
    case MediaNodeEvent::NewAudioGraph:
        if (m_audioNode)
            m_audioNode->setGraph(static_cast<AudioGraph *>(event->data()));
        break;
    ...
    if (m_audioNode)
        m_audioNode->mediaNodeEvent(event);
