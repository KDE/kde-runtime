...
QStringList BackendInfo::quickTimeCompressionFormats()
{
    ...
    for (CFIndex i=0; i<count; ++i){
        const CFStringRef name = (const CFStringRef) CFArrayGetValueAtIndex(*formats, i);
...
QStringList BackendInfo::coreAudioCodecs(Scope scope)
{
    ...
    for (UInt32 i=0; i<numFormats; ++i){
        ...
        BACKEND_ASSERT3(err == noErr, "Could not get awailable decoder/encoder names", NORMAL_ERROR, result)
        result << QCFString::toQString(name);
    }
    return result;
}
